...
8. **LiveVariable JSON must be Studio's FULL envelope** (§7): `"type"` is the **entity simple name**
   (`"Category"`), NOT the fully-qualified class (FQN goes in `"package"` /
   `propertiesMap.fullyQualifiedName`); include `tableName`, `entityName`, `dataBinding`, `matchMode`,
   `orderBy`, and full per-column metadata (`columnName`/`length`/`precision`/`generator`) from the
   published data model. Column `type` for a double is `"double"` (not `float`); dates → `"timestamp"`.
9. **Styling is via component variants** — styled widgets carry **both** `variant="<name>"` and
   ...
   `width/height = fill|hug|%|px`. **Derive the palette FROM the legacy app's CSS/SCSS** (§8·0), not a
   fresh one; palette edits go in **both** `app.override.css` `:root` and `color.light.json`. (§8–§10)
...
- **Don't silently defer the three most-skipped tasks** — a "complete" migration includes them, and
  they are exactly what a full run tends to drop:
  1. **Theme** — pull the palette/fonts FROM the legacy CSS/SCSS so it matches (design-tokens.md §8·0),
     don't invent colors.
  2. **Custom Java service** — author the non-CRUD backend (checkout/business logic) with its §13a
     wiring; don't ship a dead button (data-variables.md §13).
  3. **Security** — actually enable it (`enforceSecurity: true` + DATABASE provider); prepared
     `intercept-urls`/`roles` alone do nothing (security.md §6e).
  A datasource `readOnly:true` flag does NOT justify staging — it gates DDL, not DML, so writes still
  persist (data-variables.md §4b). Only stage when the database is genuinely non-writable, and then say
  so explicitly and keep the task open — never let it read as done.
